<div class="nb-manage-controls">
    <table class="form-table">
        ...
        <tbody>
            <tr>
                <td>
                    <div class="nb-listelem-outer manage-entry" style="background-color: #ddd;">
                        <p style="margin-bottom: 5px;">Tipp: Wähle mit <i class="fa-solid fa-eye nb-ferien-iconpreview"></i>, welche Ferien den <u>Nutzern</u> angezeigt werden. Wähle mit <i class="fa-solid fa-heart nb-ferien-iconpreview"></i> welche Ferien <u>für dich beim Bearbeiten</u> standardmäßig ausgewählt werden.</p>
                    </div>
                </td>
            </tr>
            ...
            <tr>
                <td>
                    <div class="nb-listelem-outer manage-entry <?= $row->ACTIVE ? "nb-ferien-active" : "" ?> <?= $row->ENDDATE < $curDate ? "nb-list-past" : ($row->STARTDATE > $curDate ? "nb-list-future" : "nb-list-current") ?>"
                        data-id="<?= $row->FID ?>">
                        <div class="nb-listelem-inner-title">
                            <p class="title"><a
                                    href="?page=nb-options-menu&action=fkurs-manage&fe=<?= $row->FID ?>"><?= $row->LABEL; ?></a>
                            </p>
                            <?php  ?>
                            <small><?= sprintf("%02d.%02d.%d", $sd[2], $sd[1], $sd[0]); ?>
                                -
                                <?= sprintf("%02d.%02d.%d", $ed[2], $ed[1], $ed[0]); ?></small>
                        </div>

                        <div class="nb-listelem-inner-modify">
                            <a class="button button-primary fe-active-course" title="Ferien de-/aktivieren" href="#">
                                <i
                                    class="fa-solid <?= $row->ACTIVE ? "fa-eye" : "fa-eye-slash" ?>"></i>
                            </a>
                            <a class="button <?= $thisStandard ? "button-green" : "button-primary" ?> fe-standard-course"
                                title="Standardferien setzen" href="#">
                                <i
